	...
	<div v-if="isRenote" :class="$style.renote">
		...
		<div :class="$style.renoteInfo">
			...
			<span v-else-if="note.visibility !== 'public'" style="margin-left: 0.5em;" :title="i18n.ts._visibility[note.visibility]">
				<i v-if="note.dontShowOnLtl === true" class="ti ti-cloud"></i>
...
import { computed, inject, onMounted, ref, reactive, useTemplateRef, watch, provide, shallowRef } from 'vue';
...
const instanceName = appearNote.user.host;
const isMFMSilence = ref(false);
const text = ref(appearNote.text);
...
function mfmParse() {
	...
	// 	return computed(() => appearNote.text ? mfm.parse(appearNote.text) : null);
	// }
}
